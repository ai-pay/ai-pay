import {
  AiApiResponse, ChatCompletionRequest, ChatCompletionStreamChunk
 ...
 * 
 * @param {(response: ChatCompletionStreamChunk): void} callback - The callback function to stream chunks of the response. 
 * 
...
export async function chatCompletionStream(
  ...
  callback: (response: ChatCompletionStreamChunk) => void, 
  options: RequestOptions = {},
): Promise<AiApiResponse<object>> {

  const url = AiUsageUrl() + "/chat-completion-stream";
...
type CallbackType = (response: ChatCompletionStreamChunk) => void
